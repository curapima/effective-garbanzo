...
export function PostCard({ post }: { post: Post }) {
  return (
    <article className="group border border-kimi-border bg-kimi-panel p-5 shadow-pixel transition hover:-translate-y-1 hover:border-kimi-green">
      <div className="mb-4 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.18em] text-kimi-muted">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {post.tags.map((tag) => (
          <Link
            ...
            className="border border-kimi-border bg-kimi-black px-2 py-1 text-xs text-kimi-muted hover:border-kimi-green hover:text-kimi-green"
          >
            #{tag}
          </Link>
        ))}
