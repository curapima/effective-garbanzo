  ...
  return (
    <div>
      <section className="mx-auto grid min-h-[calc(100vh-65px)] max-w-6xl items-center gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <p className="mb-4 inline-flex border border-kimi-border bg-kimi-panel px-3 py-2 font-pixel text-xs uppercase tracking-[0.26em] text-kimi-green">
            CRT BLOG OS / READY
          </p>
          <h1 className="max-w-3xl font-pixel text-4xl font-black leading-tight text-kimi-text sm:text-6xl">
            effective-garbanzo
          </h1>
      ...
      <section className="border-y border-kimi-border bg-kimi-panel/70">
        <div className="mx-auto grid max-w-6xl gap-4 px-4 py-8 sm:grid-cols-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-pixel text-2xl font-black">{posts.length}</p>
              <p className="font-pixel text-sm text-kimi-muted">文章信号</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <FolderKanban className="text-kimi-green" aria-hidden="true" />
            <div>
              <p className="font-pixel text-2xl font-black">{categories.length}</p>
              <p className="font-pixel text-sm text-kimi-muted">分类频道</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Tags className="text-kimi-green" aria-hidden="true" />
            <div>
              <p className="font-pixel text-2xl font-black">{tags.length}</p>
              <p className="font-pixel text-sm text-kimi-muted">标签索引</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 font-pixel text-3xl font-black">精选文章</h2>
          </div>
